...
export function namespaceFindings(
  ...
  findings.push(
    observations.denied_entries_visible === 0
      ? {
          boundary: 'filesystem',
          verdict: 'proven',
          evidence:
            ...
            'tree is absent rather than merely unreferenced',
        }
      : {
          boundary: 'filesystem',
          verdict: 'violated',
          evidence:
            `${String(observations.denied_entries_visible)} entr(ies) were visible under a ` +
            `denied root (${deniedRoots}) from inside the namespace`,
        },
  );
    ...
    refused
      ? {
          ...
          evidence:
            ...
            `${observations.control_host} was ${observations.control_result}`,
        }
      : {
          boundary: 'network',
          verdict: 'violated',
