...
| O-4                 | **SQLite binding** (`node:sqlite` vs `better-sqlite3`).                                                                                                                                                                  | Still not chosen, but no longer for want of evidence. As of 2026-09-05 `node:sqlite` has all seven checks observed on **both** required platforms; `better-sqlite3` on Linux only. **Open question for the owner**, stated in the decision log §4c. | The candidate with complete coverage is the one the frozen guide did _not_ name in D18.5, and its own runtime still flags it experimental. That trade-off is the owner's to make, not this plan's. |
...
| B7  | SQLite binding qualification                             | **PARTIAL** | All seven recorded checks executed against **both** candidates on Linux; all seven pass for each. `better-sqlite3@13.0.3`'s engine was measured **through the binding** (3.53.4), never substituted from Node's. **win32 is now observed for `node:sqlite` only**: the qualification suite ran on the Windows runner on 2026-09-05 (`6d8c27b`) and all seven checks passed there, check 4 genuinely contending for a lock (13.3 s). `better-sqlite3` is deliberately not a dependency, so CI cannot resolve it and **its win32 coverage remains unobserved**. **Still no binding is selected**: that is now an owner decision, not a missing measurement — see O-4.                                                                                            |
...
**The external blocker is gone; what remains is a decision.** The Windows
observation that B1 and B7 were both waiting on has now happened (2026-09-05,
`6d8c27b`). B1 is closed on it. B7 is not, but no longer for want of running
something: `node:sqlite` now has all seven checks observed on both required
platforms, while `better-sqlite3` has them on Linux only, because it is
deliberately not a dependency of this repository and CI therefore cannot load
it. Choosing between "the candidate with complete observed coverage" and "the
candidate the frozen guide named in D18.5" is an owner decision with a real
trade-off — `node:sqlite` is still flagged experimental by its own runtime — so
it is put to the owner rather than settled here (O-4). B2's remaining gap is a
Stage 3 activity by the guide's own sequencing, not a Stage 0 one.
