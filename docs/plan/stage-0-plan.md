...
| O-4 (closed)           | **SQLite binding** (`node:sqlite` vs `better-sqlite3`).                                                                                                                                                                                         | **Closed 2026-09-05: `node:sqlite` adopted** by owner decision, on the qualification evidence — complete observed coverage on both required platforms, no external or native dependency, identical measured engine. Recorded as approved deviation **C-9**. | Resolved on measurement plus an explicit owner approval, not by authority alone. The deviation stays visible as a deviation.                                |
| O-5 — **CLOSED by B4** | **Capability taxonomy seed (D28).** The guide seeds `contracts/capabilities.yaml` from the legacy repository's `core/capability-registry.yaml`. It proved reachable later in Stage 0, and B4 seeded 28 ids verbatim at revision `4d517840f18d`. | The file exists with a valid schema and an explicitly empty, provenance-marked seed. No capability ids are invented.                                                                                                                                        | Inventing a taxonomy would create exactly the unprovenanced canonical knowledge the constitution forbids.                                                   |
...
| --- | -------------------------------------------------------- | ----------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| B1  | Linux CI + Windows smoke; D35 cross-platform digest gate | **PASS**    | **Observed on GitHub Actions, 2026-09-05, commit `6d8c27b` (PR #1).** `linux (primary)`, `fitness` and `windows smoke (D35 cross-platform digest)` all green. The gate is not the green tick but the comparison inside it, and it printed: linux `sha256:e8c450ac…ee6febbe`, windows `sha256:e8c450ac…ee6febbe`, _"identical on both platforms"_ — matching the fixture pinned in `snapshot.test.ts`. The vacuity guard ran on the Windows runner too (10 files, 283 tests), so the comparison was not made over an empty suite. The **first** attempt (`368467b`) failed on Windows at the toolchain doctor — a defect in the check, not the contract — and is recorded in the status record; this row reports the run that actually executed the comparison. |
...
| B7  | SQLite binding qualification                             | **PASS**    | All seven recorded checks executed against **both** candidates on Linux; all seven pass for each. `node:sqlite` also passes all seven on **win32** (2026-09-05, run 33953023783; per-check evidence in `qualification/evidence/sqlite-qualification-win32.md`), so it has the complete platform coverage check 3 names. `better-sqlite3@13.0.3`'s engine was measured **through the binding** (3.53.4), never substituted from Node's; its win32 coverage stays unobserved because it is deliberately not a dependency here. **Binding selected: `node:sqlite`**, by owner decision on 2026-09-05, recorded as approved deviation **C-9** from D18.5. O-4 closed.                                                                                              |
...
**Both remaining blockers are resolved.** The Windows observation that B1 and
B7 were waiting on happened on 2026-09-05, and the owner answered the two
questions it raised:

- **O-4 — SQLite binding.** `node:sqlite` adopted, recorded as approved
  deviation **C-9** from D18.5. The framing is deliberate and preserved in the
  decision log: D18.5 is not treated as having been wrong. It chose
  `better-sqlite3` on the information available when the guide was frozen;
  Stage 0 then produced qualification evidence that did not exist at that time,
  and the owner approved the deviation **on that evidence**.
- **The F1–F12 gate reading**, recorded as **C-10**. Every rule enforceable at
  this stage must be green; every rule whose subject belongs to a later stage
  must be explicitly dormant **and** carry a guard that fails the moment its
  subject appears. `fitness/checks/dormancy.test.ts` is that guard, and it was
  negative-controlled: creating `packages/launcher` or `simulations/` makes it
  fail until the corresponding rule is armed.

B2's remaining gap is a Stage 3 activity by the guide's own sequencing, not a
Stage 0 one. Stage 0 now closes on the harness-generated report
(`pnpm report:stage0`), not on this document.

### C. Measured result

Measured on Linux x64, Node 24.20.0, pnpm 11.25.0:

- **24 test files, 591 tests, all passing.** Two consecutive full runs produce
  identical results.
...
- Boundary analysis: **84 modules, 238 dependencies cruised**, 0 violations —
...
- Windows: **observed.** GitHub Actions `windows-latest`, Node 24.20.0,
  2026-09-05 (`6d8c27b`): core determinism 234 tests, bootstrap snapshot 29
