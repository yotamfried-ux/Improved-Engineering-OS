...
- Streamable HTTP requests also carry `MCP-Protocol-Version`, `Mcp-Method` and
  `Mcp-Name` headers. Stage 1 uses stdio, so these are noted, not implemented.
...
| `packages/adapters/mcp` | Stateless server exposing the four Agent Contract tools, `server/discover`, `ttlMs`/`cacheScope` on lists, `readOnlyHint` on the three reads and `idempotentHint` on `observe`.                                       | HTTP transport, OAuth, sessions.                                             |
| Ranking                 | Not implemented. `resolve`/`expand` return the contract shape over an index that has no assets yet.                                                                                                                   | D20.3 deterministic ranking arrives with `packages/resolver` at **Stage 2**. |
...
| G4  | Self-describing `_meta` is accepted on every request; a version mismatch yields `-32022`.                           |
| G5  | `tools/list` carries `ttlMs`, and `tools/call` works for all four tools.                                            |
