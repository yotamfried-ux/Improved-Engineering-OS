...
`bash <dir>` exits **126**. So the _absent_ wrapper denied, while a readable directory at the same
...
contract is _never block_ is defined to warn and exit 0, so any non-zero status makes it report a
